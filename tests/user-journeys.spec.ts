...
test.describe('TailwindTrainer - Jornadas do Usuário', () => {
  ...
  test('Jornada 1: Usuário pode navegar pela homepage e acessar seções principais', async ({ page }) => {
  ...
  test('Jornada 2: Usuário pode acessar grupos de aprendizado', async ({ page }) => {
    // Navegar para Learn
  ...
  test('Jornada 3: Usuário pode completar um quiz básico', async ({ page }) => {
    ...
    const questionElement = page.locator('[data-testid="current-question"], .question, h3, h2').first();
    ...
    await expect(page.locator('text=Correct, text=Incorrect, text=Next Question')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('Jornada 5: Challenge Free Practice funciona corretamente', async ({ page }) => {
  ...
  test('Jornada 6: Navegação para páginas de ajuda', async ({ page }) => {
    ...
    await expect(page.locator('text=TailwindCSS, text=learn, text=help, text=guide')).toBeVisible();
  });
  ...
  test('Jornada 8: Verificar leaderboard', async ({ page }) => {
    ...
    await expect(page.locator('text=Leaderboard, text=Ranking, text=Score')).toBeVisible();
  });
});
