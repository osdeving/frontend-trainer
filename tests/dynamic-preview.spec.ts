...
test.describe('TailwindTrainer - Dynamic Preview Feature', () => {
    ...
    test('Dynamic preview component is visible on practice challenges', async ({ page }) => {
    ...
    test('Preview updates when user types Tailwind classes', async ({ page }) => {
        ...
        await expect(page.locator('text=Match!').or(page.locator('text=Different'))).toBeVisible();
    });

    test('Compare mode toggles correctly', async ({ page }) => {
        // First, enter some answer to enable compare button
        await page.fill('input[placeholder*="TailwindCSS"]', 'flex justify-center');
    ...
    test('CSS output is displayed when user enters classes', async ({ page }) => {
        // Enter some Tailwind classes
        await page.fill('input[placeholder*="TailwindCSS"]', 'bg-blue-500 p-4 rounded');
    ...
    test('Dynamic preview works with complex Tailwind classes', async ({ page }) => {
        // Test with more complex Tailwind combinations
        await page.fill('input[placeholder*="TailwindCSS"]', 'flex items-center justify-between bg-blue-100 p-4 rounded-lg shadow-md');
    ...
    test('Preview component handles empty input gracefully', async ({ page }) => {
    ...
    test('Preview survives navigation between questions', async ({ page }) => {
        ...
        await expect(page.locator('text=Show Preview').or(page.locator('text=Hide Preview'))).toBeVisible();
    });
});
