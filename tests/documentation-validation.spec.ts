...
test.describe('TailwindTrainer - Validação de Documentação e Help', () => {
  test('Help page deve listar funcionalidades principais mencionadas no README', async ({ page }) => {
  ...
  test('Todas as rotas principais devem estar acessíveis', async ({ page }) => {
    const routes = [
      '/',
      '/learn',
      '/challenges', 
  ...
  test('Grupos de aprendizado mencionados no README devem existir', async ({ page }) => {
  ...
  test('Challenges mencionados no README devem existir', async ({ page }) => {
    ...
    for (const challenge of expectedChallenges) {
      await expect(page.locator(`text=${challenge}`).first()).toBeVisible();
    }
  });

  test('Sistema de aprendizado progressivo deve funcionar', async ({ page }) => {
  ...
  test('Sistema de gamificação deve ter elementos visíveis', async ({ page }) => {
  ...
  test('Responsividade mencionada no README deve funcionar', async ({ page }) => {
    // Testar desktop
  ...
  test('Sistema de preview mencionado deve funcionar', async ({ page }) => {
    ...
    if (await previewButton.first().isVisible().catch(() => false)) {
      ...
      await expect(page.locator('[data-testid="preview"], .preview')).toBeVisible();
    }
  });
  ...
  test('Sistema de hints mencionado deve funcionar', async ({ page }) => {
    ...
    if (await hintButton.first().isVisible().catch(() => false)) {
      ...
      await expect(page.locator('[data-testid="hint"], .hint, text=Think')).toBeVisible();
    }
  });
});
