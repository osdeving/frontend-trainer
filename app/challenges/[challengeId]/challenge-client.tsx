...
export default function ChallengeClient({ challenge, questions }: ChallengeClientProps) {
  ...
  const generateHint = (tailwindClass: string, category: string) => {
    const hints = {
      ...
      'Font Size': 'This controls text size - remember Tailwind uses t-shirt sizing.',
  ...
  const checkAnswer = () => {
    ...
    if (isAnswerCorrect) {
      const baseXP = currentQuestion.difficulty === 'easy' ? 10 : 
                    currentQuestion.difficulty === 'medium' ? 15 : 20;
      
      // Bonus XP for challenges
      const bonusMultiplier = challenge.type === 'timed' ? 1.5 : 
                             challenge.type === 'survival' ? 1.3 : 
                             challenge.type === 'themed' ? 1.2 : 1;
  ...
  const completeChallenge = () => {
    ...
    challengeStore.completeChallenge(challenge.id, score, totalTime, accuracy);
  ...
  if ((challenge.type === 'survival' && lives === 0) || 
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center">
        <Card className="w-full max-w-md text-center border-0 shadow-xl">
          <CardContent className="pt-8 pb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Challenge Failed!</h2>
            <p className="text-gray-600 mb-4">
              {challenge.type === 'survival' ? 'You ran out of lives!' : 'Time\'s up!'}
            </p>
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-gray-600">{score}</div>
                  <div className="text-sm text-gray-700">Final Score</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-gray-600">{correctAnswers}/{totalQuestions}</div>
                  <div className="text-sm text-gray-700">Correct</div>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              <Button onClick={() => window.location.reload()} className="w-full bg-gradient-to-r from-blue-500 to-purple-600">
          ...
          <CardContent className="pt-8 pb-6">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 mb-6">
              <div className="grid grid-cols-2 gap-4 text-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-orange-600">{correctAnswers}/{totalQuestions}</div>
                  <div className="text-sm text-orange-700">Correct</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              ...
              <div className="text-2xl font-bold text-yellow-700">{challenge.rewards.xp} XP</div>
            ...
            <div className="flex items-center space-x-4">
              ...
              {challenge.type === 'survival' && (
                <div className="flex items-center space-x-1">
                  {Array.from({ length: challenge.config.lives || 3 }).map((_, i) => (
                    <Heart 
                      key={i} 
                ...
                <div className="mb-4">
                  ...
                  {showPreview && (
                    <div className="bg-white border-2 border-dashed border-gray-300 rounded-lg p-4 flex items-center justify-center">
                      <div 
                        style={generatePreview(currentQuestion.css)}
          ...
          <Card className="border-0 shadow-xl">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {showHint && !showResult && challenge.config.allowHints && (
                ...
                <div className="flex space-x-3">
                  {!showResult ? (
                    ...
                    >
                      {currentQuestionIndex < totalQuestions - 1 ? 'Next Question' : 'Complete Challenge'}
                    </Button>
                  )}
                </div>
