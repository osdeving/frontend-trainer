...
import { Eye, EyeOff, ArrowLeftRight, Check, X } from 'lucide-react';
...
export const DynamicPreview: React.FC<DynamicPreviewProps> = ({
    ...
    const isMatch = userAnswer.toLowerCase().trim() === expectedAnswer.toLowerCase().trim();
    ...
    return (
        <div className={`space-y-4 ${className}`}>
            ...
            <div className="flex items-center justify-between">
                ...
                {userAnswer.trim() && (
                    <Badge variant={isMatch ? "default" : "secondary"} className={`gap-1 ${isMatch ? 'bg-green-500 text-white' : ''}`}>
                    ...
                    {showComparison ? (
                        ...
                        <Card>
                            ...
                            <CardContent className="flex items-center justify-center p-6">
                                <div 
                                    style={userAnswer.trim() ? userStyle : expectedStyle}
                                    className="relative"
                                >
                                    <span className="text-xs">
                                        {userAnswer.trim() ? 'Your Style' : 'Target Style'}
                                    </span>
                                    <div className="absolute -bottom-6 left-0 right-0 text-xs text-center text-muted-foreground">
